...
var SyntaxReg = (function(){
	...
	function compilePtn(source){
		...
		if (text[0] == '(' && text[text.length-1] == ')'){
			...
			try {
				...
				throw tea.error(new Error(), [source, source.indexOf(text), text], 'Syntax RegExp parse error');
			}
		}else {
	...
	function matchCallPattern(ptn, src, __r, __m){
		...
		throw tea.error(new Error(), [this.source, -1, name]);
	}
	...
	function matchBad(ptn, src, o_index, __m){
		...
		if (ptn.param.err && !ptn.param.try && !this.try){
			throw tea.error(new Error(), err_token, 'Syntax parse error');
		}
		return false;
	}

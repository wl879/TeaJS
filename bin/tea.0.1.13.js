...
(function (){
	...
	Module.register('/Users/wl/Sites/TeaJS/lib/tea.js', '../lib/tea.js', function(exports, require, module, __filename, __dirname){
		require("../lib/tools/utils.js");
		global.tab_size = '    ';
		global.tea = module.exports;
		tea.teapath = Path.dirname(__filename);
		tea.argv = require("../lib/argv.js");
		tea.helper = require("../lib/helper.js");
		tea.error = require("../lib/error.js");
		tea.preprocess = require("../lib/preprocess/index.js");
		tea.syntax = require("../lib/syntax/index.js");
		tea.rewriter = require("../lib/rewriter/index.js");
	...
	});Module.register('/Users/wl/Sites/TeaJS/lib/tools/utils.js', '../lib/tools/utils.js', function(exports, require, module, __filename, __dirname){
		var _id_index = 0;
		global.Path = require("../lib/tools/path.js");
		global.Hash = require("../lib/tools/hash.js");
		global.Text = require("../lib/tools/text.js");
		global.tab_size = '    ';
		global.ID = function(){
	...
	});Module.register('/Users/wl/Sites/TeaJS/lib/tools/path.js', '../lib/tools/path.js', function(exports, require, module, __filename, __dirname){
		...
		module.exports = Path;
	});Module.register('/Users/wl/Sites/TeaJS/lib/tools/hash.js', '../lib/tools/hash.js', function(exports, require, module, __filename, __dirname){
		var Crypto, Hash;
		Crypto = require('crypto');
		Hash = function(arr, separator){
			var h = {}, keys, extend;
		...
		module.exports = Hash;
	});Module.register('/Users/wl/Sites/TeaJS/lib/tools/text.js', '../lib/tools/text.js', function(exports, require, module, __filename, __dirname){
		var _pair_token,
			_pair_re,
			Text,
			Fs = require('fs'),
		...
		Text.split = function(text, separator, trim, del_esc){
			var slice, a, m, b, p, ab;
			if (!separator){
				separator = /(\\*)(\ )/;
			}else if (typeof separator == 'string'){
				separator = new RegExp('(\\\\*)('+separator.replace(/(\W)/g, '\\$1')+')');
			}else {
			...
			while (text){
				if (m = text.substr(a).match(separator)){
					...
					if (p = _pair_re.exec(text)){
						ab = this.indexPair(text, p.index, p[0], _pair_token[p[0]], true);
						if (ab && b > ab[0]){
							a = ab[1]+1;
							continue;
						}
					}
					slice.push(text.slice(0, b));
			...
			var ret = [];
			for (var i = 0, v; i < slice.length; i++){
				v = slice[i];
				if (trim){
					v = v.trim();
				}
		...
		Text.print = function(data, ret_str, __circular){
			if (__circular == null){
				__circular = [];
			}
			var str, class_name, list, array_mode;
			if (typeof data == 'string'){
				...
				class_name = isClass(data);
				if (class_name == 'RegExp'){
					str = data+'';
				}else {
					list = [];
					str = '';
					array_mode = data.hasOwnProperty('length');
					if (__circular.indexOf(data) !== -1){
						return '[circular]';
					}
					...
					for (var k in data){
						...
						if (array_mode && /\d+/.test(k)){
							list.push(Text.print(data[k], true, __circular));
						}else {
							list.push(k+' : '+Text.print(data[k], true, __circular));
						}
					}
					if (list.length){
						str = '\n'+list.join(',\n').replace(/^/mg, '\t')+'\n';
		...
		Text.getName = function(text){
			if (text){
				...
				return name.replace(/(?:^|[^a-zA-Z0-9\$]+)(\w)/g, function($0, $1){
					return $1.toUpperCase();
				});
			}
		};
		module.exports = Text;
	});Module.register('/Users/wl/Sites/TeaJS/lib/argv.js', '../lib/argv.js', function(exports, require, module, __filename, __dirname){
		var Argv = (function(){
		...
		module.exports = new Argv(null, null, "r{** g{Tea} test w{script help} *************************************************************}\n\
  # parameter:\n\
    -f,--file  <file>                  输入文件\n\
    ...
    --debug                            显示调试信息 [log prep syntax write all]");
	});Module.register('/Users/wl/Sites/TeaJS/lib/helper.js', '../lib/helper.js', function(exports, require, module, __filename, __dirname){
		var grammar_log_cache;
		require("../lib/tools/debug.js");
		...
		grammar_log_cache = [];
		debug.addEvent('grammar', function(type, name, ref){
			var index, slice, text;
			if (type == '>'){
				if (grammar_log_cache.length == 0){
					print('+ [grammar debug]');
				}
				grammar_log_cache.push(name);
			}else {
				if (!ref){
					grammar_log_cache.pop();
				}
				if (/^(Statement|Expression|.*Block)$/.test(name)){
					index = grammar_log_cache.lastIndexOf(name);
					slice = grammar_log_cache.splice(index);
					if (ref){
						text = slice.join(' >').replace(/( >\w+)(\1)+/g, function($0, $1){
							return $1+'('+Math.floor($0.length/$1.length)+')';
						});
						print.stdout('    '+text+'\n');
						print.stdout(print.toString(ref).replace(/^/mg, '        | '));
						print.stdout('\n');
					}
					if (index == 0){
						print.stdout('\n');
		...
		debug.addEvent('scope', function(scope){
			print(scope);
		});
		debug.addEvent('std', function(){
			debug.echo(print.toString(arguments));
		});
		...
		function nodePrinter(node, _level, _indent){
			if (_level == null){
				_level = 0;
			}
			if (_indent == null){
			...
			var isNode, text;
			if (/root|node|block/i.test(node.type) && node.length){
				isNode = '\n'+print.strc(tab_size, _indent+1);
			}else {
				isNode = '';
			}
			text = "[(g{"+((node._scope && node._scope.node == node ? '*' : '')+node.type)+"}) ";
			for (var i = 0, item; i < node.length; i++){
				...
				if (node[i].istoken){
					...
					text += isNode+nodePrinter(node[i], _level+1, isNode ? _indent+1 : _indent);
				}
			}
			text += (isNode ? '\n'+print.strc(tab_size, _indent) : '')+']';
			if (_level == 0){}
			return text;
		}
		function scopePrinter(scope, __level){
			...
			for (var key in scope){
				...
				var item = scope[key];
				if (/^(node|top|_top_|cache|parent|type|name|argumentsDefine)$/.test(key) || item == null){
					continue;
				}
				switch (key){
					case 'variables':case 'members':
						var v_types = {}, v_text = [];
						...
						for (var type in v_types){
							...
							if (!v_types.hasOwnProperty(type)){
								continue;
							}
					...
					case 'subs':
						...
						for (var k = 0; k < item.length; k++){
							sub_text.push(scopePrinter(item[k], __level+1));
						}
						if (sub_text.length){
							texts.push(key+' : [\n'+sub_text.join(',\n').replace(/^/mg, tab_size)+']');
						}
			...
			text = "* Scope : "+(scope.type+(scope.name ? ' : '+scope.name : ''))+"\n"+(texts.join('\n').replace(/^/mg, tab_size));
			if (__level == 0){
				return text.replace(/^/mg, '| ').replace(/\|(\s*)(.*)$/, function($0, $1, $2){
					return '|'+print.strc('_', $1.length-1)+' '+$2;
				})+'\n';
			}
			return text;
		}
		function patterPrinter(patt, __level){
			if (__level == null){
				__level = 0;
			}
			var text, texts;
			switch (isClass(patt)){
				case 'PatternData':
					switch (patt.type){
						case '*':case 'RegExp':case 'Code':case 'Tokn':case 'Node':
							text = '"'+patt.pattern+'"';
							break;
						case 'CodeList':
							text = '["'+patt.pattern.join('" "')+'"]';
							break;
						case 'Call':
							text = '*'+patt.pattern+'('+patt.param.join(', ')+')'+'*';
							break;
						case 'Sub':
							text = '\n'+patterPrinter(patt.pattern, __level+1)+'\n';
							break;
					}
					texts = [patt.type+': '+text];
					if (patt.remode){
						texts.push(' remode:"'+patt.remode+'"');
					}
					if (patt.info.name){
						texts.push(' name:"'+patt.info.namemode+patt.info.name+'"');
					}
					if (patt.info.test){
						texts.push(' test:true');
					}
					text = '<'+texts.join(',')+'>'+' <--> str:"'+patt.string+'"';
					break;
				case 'Pattern':
					texts = [];
					for (var i = 0; i < patt.length; i++){
						texts.push(patterPrinter(patt[i], __level+1));
					}
			...
			if (__level === 0){
				text = '+ [Pattern '+(patt.grammarName || '')+']\n+ "'+patt.string+'"\n'+text.replace(/\| \- or/g, '|-- or')+'\n';
			}
			return text;
		}
		...
		function logString(something){
			var texts;
			if (isArray(something) && something.length){
				if (something[0].istoken){
					texts = [];
					for (var _i = 0, tk; _i < something.length; _i++){
						tk = something[_i];
						if (tk){
							texts.push(tk.text);
						}
			...
			return print.toString([something]);
		}
		...
		print.register('Pattern', patterPrinter);
		print.register('PatternData', patterPrinter);
	});Module.register('/Users/wl/Sites/TeaJS/lib/tools/debug.js', '../lib/tools/debug.js', function(exports, require, module, __filename, __dirname){
		...
		debug.__defineGetter__('level', function(){
			return debug_lv;
		});
	...
	});Module.register('/Users/wl/Sites/TeaJS/lib/tools/printer.js', '../lib/tools/printer.js', function(exports, require, module, __filename, __dirname){
		require("../lib/tools/utils.js");
		...
		print.toText = function(){
			...
			text = text.replace(/\((.*) x(\d+)\)/g, function($0, $1, $2){
				return print.strc($1, parseInt($2));
			});
			text = text.replace(/\[border\:(.*?)(\:end\]|$)/g, function($0, $1){
				return print.border($1);
			});
		...
		print.line = function(text){
			if (/^([\W\ x]){4}$/mg.test(text)){
				...
				text = text.replace(/^([\W\ x])\1{3}$/mg, function($0, $1){
					return print.strc($1, the_width);
				});
			}
			return text;
		};
		print.flex = function(text, min_width){
			...
			while (/^(.*?)(?:<-{1,2}>)(.*)$/mg.test(text)){
				...
				text = text.replace(/^(.*?)(?:<-{1,2}>)(.*)$/mg, function($0, $1, $2){
					return $1+print.strc(' ', col_width-$1.length)+$2;
				});
			}
			return text;
		};
	...
	});Module.register('/Users/wl/Sites/TeaJS/lib/error.js', '../lib/error.js', function(exports, require, module, __filename, __dirname){
		var code, helper = require("../lib/helper.js");
		code = {0 : 'Tea core not init',
			1 : 'not supported $$',
			1001 : 'Identifier "$$" has already been declared',
			1002 : 'The $$ declaration statement need at class inward',
			1003 : 'identifier "$$" is an const declaration, cant modify',
			1004 : 'nonsupport rest variable',
			1005 : 'class expression need a name',
			2001 : 'for statiment of condition index parse error!',
			5001 : 'reader pattern syntax error',
			5002 : 'reader pattern "$$" method undefined',
			5003 : 'reader pattern parse error',
			5004 : 'reader condition "$$" method undefined',
			5005 : 'reader condition syntax error',
			6001 : 'must be capitalize the first letter in standard parser',
			6006 : 'standard read method params type error',
		...
		function creater(err, msg, target, name, err_shift){
			var sub, stacks, error;
		...
		function toString(err){
			...
			var texts = [], stacks = [];
			texts.push(err.name);
			stacks.push(debug.stacksToText(err.stacks));
			if (err.target){
				texts.push(point(err.target, err.message).replace(/^/mg, '\t'));
			}else {
				texts.push(err.message);
			}
			var top = err.top, top_texts = [];
			while (top){
				top_texts.push('   '+top.name);
				if (top.target){
					top_texts.push(point(top.target, err.message).replace(/^/mg, '\t'));
				}else {
		...
		function point(something, msg){
			var cache;
			switch (isClass(something)){
				case 'Standard':
					cache = something.cache;
					return pointByText(cache._standard+' : '+cache._condition+' : '+cache._pattern, 'xxx', null, null, msg);
				case 'Pattern':case 'PatternData':
					return pointByText(something.root.string, something.string, null, null, msg);
				case 'Ast':case 'Node':case 'Source':case 'Token':
					return point(getLocation(something), msg);
				case 'Location':
					var text = something.source,
						pos = something.start,
						code = text.slice(something.start, something.end+1),
						file = something.fileName;
					return pointByText(text, pos, code, file, msg);
				case 'Array':
					return pointByText(something[0], something[1], something[2], something[3], msg);
			}
		}
		function pointByText(text, pos, code, file, msg){
			var line, num, col, pot_num, pot_shift, pot_line, qq_mark, pot_echo, _ref;
			if (typeof pos == 'string'){
				file = code, code = pos, pos = -1;
			}
			if (pos == -1 && code){
			...
			_ref = Text.indexLine(text, pos), line = _ref[0], num = _ref[1], col = _ref[2];
			if (code){
				line = line.substr(0, col)+print.color('#r{'+code+'}')+line.substr(col+code.length);
			}
			pot_num = num+' | ';
			pot_shift = (pot_num+line.substr(0, col)).replace(/[^\s]/g, ' ')+print.strc('^', code.length);
			pot_line = line.replace(/\n/, '\\n');
			qq_mark = (/(?:[^\\]|^)"/).test(line) ? "'" : '"';
	...
	});Module.register('/Users/wl/Sites/TeaJS/lib/preprocess/index.js', '../lib/preprocess/index.js', function(exports, require, module, __filename, __dirname){
		var template, Context = require("../lib/preprocess/context.js");
		template = require("../lib/preprocess/template.js");
	...
	});Module.register('/Users/wl/Sites/TeaJS/lib/preprocess/context.js', '../lib/preprocess/context.js', function(exports, require, module, __filename, __dirname){
		var Context = (function(){
				var Processor = require("../lib/preprocess/processor.js"),
					Macro = require("../lib/preprocess/macro.js"),
					Sugar = require("../lib/preprocess/sugar.js"),
					Template = require("../lib/preprocess/template.js"),
					DefPor,
					Syntax,
					Require,
				...
				function Context(argv, extend){
					if (!Syntax){
						Syntax = require("../lib/syntax/index.js");
					}
					if (!Require){
						Require = require("../lib/preprocess/require.js");
					}
					if (!ReWriter){
						ReWriter = require("../lib/rewriter/index.js");
					}
					this.macro = {"map" : []};
					this.macrofun = {"map" : []};
					this.statement = {"map" : []};
				...
				Context.prototype.template = function(type, str){
					var re, m, val;
					switch (type){
						case 'string':
							re = /([^\\]|^)(\#\{((\w+)(.*?))\})/g;
							while (m = re.exec(str)){
								val = Processor.macroText(ctx, m[3], 0, m[4]);
				...
				Context.prototype.echo = function(output, outmap){
					...
					if (!output){
						output = this.argv && this.argv.out;
					}
	...
	});Module.register('/Users/wl/Sites/TeaJS/lib/preprocess/processor.js', '../lib/preprocess/processor.js', function(exports, require, module, __filename, __dirname){
		var Processor = (function(){
				var include_cache = {}, Template = require("../lib/preprocess/template.js");
				...
				Processor.pound = function(ctx, src, text, i, token, cache){
					...
					if (match = text.substr(i+1).match(/^([\$A-Za-z_][\w\$]*)(\:?)/)){
						type = match[1];
						if (typeof Processor[type] == 'function'){
							...
							token.text = '#'+match[0];
							token.types = ['INSTRUCTION', 'IDENTIFIER', 'CONST', 'ConstTokn'];
				...
				Processor.line = function(ctx, src, text, i, token){
					...
					token.types = ['NumTokn', 'ConstTokn', 'NUMBER', 'CONST'];
					if (token.indent >= 0){
				...
				Processor.run = function(ctx, src, text, i, token){
					...
					try {
						...
						var err_pot = tea.error.point(token);
				...
				Processor.macroText = function(ctx, text, index, code){
					return parseMacro(ctx, null, text, 0, null, code);
				};
				...
				Processor.string = function(ctx, src, text, i, token){
					...
					str = Template.string(str);
					token.text = str;
					token.types = ['StringTokn', 'ConstTokn', 'STRING', 'CONST'];
				...
				Processor.regexp = function(ctx, src, text, i, token){
					...
					if (!testValue(src, src.prevIndex(src.length, true))){
						...
						token.types = ['RegExpDecl', 'RegExpTokn', 'ConstTokn', 'REGEXP', 'CONST'];
						token.location.end = b;
					}
					return token;
				};
	...
	});Module.register('/Users/wl/Sites/TeaJS/lib/preprocess/template.js', '../lib/preprocess/template.js', function(exports, require, module, __filename, __dirname){
		function regexp(str){
			...
			if (qq == '"'){
				...
				for (var i_ref = slices(str), i = 0, item; i < i_ref.length; i++){
					...
					if (i%2){
						...
						str_slices.push('"'+escapeString(symbol, item, qq)+'"');
					}
				}
				str = str_slices.join('+');
			}else {
		...
		function escapeString(symbol, str, qq){
			switch (symbol){
				case '""""':case '`':case "''''":
					str = Text(str, qq).replace(/([^\\]|^)(\\{2,4,6,8})?\\n/g, '$1$2\\n\\\n');
					break;
				case '"""':case "'''":
					str = str.replace(/([^\\]|^)\n/g, '$1\\n').replace(/^\n/mg, '\\n\\\n');
					break;
	...
	});Module.register('/Users/wl/Sites/TeaJS/lib/preprocess/macro.js', '../lib/preprocess/macro.js', function(exports, require, module, __filename, __dirname){
		var Macro = (function(){
				var Template = require("../lib/preprocess/template.js");
				function Macro(name, params, body, parent){
					...
					if (Template.test(body)){
						try {
							this.script = Template.func(this.body, this.params, true);
						}catch (e) {
							this.error = e;
						}
					}
	...
	});Module.register('/Users/wl/Sites/TeaJS/lib/preprocess/sugar.js', '../lib/preprocess/sugar.js', function(exports, require, module, __filename, __dirname){
		var Sugar = (function(){
				var Syntax = require("../lib/syntax/index.js"),
					Template = require("../lib/preprocess/template.js");
				function Sugar(type, name, pattern, writer, parser){
					...
					this.parser = parser;
					if (Template.test(writer)){
						try {
							this.writer = Template.func(writer, ['node', 'writer'], true);
						}catch (e) {
							this.error = e;
						}
					}else {
				...
				Sugar.prototype.read = function(reader, node){
					var res, write = reader.new(this.name);
					if (typeof this.writer == 'string'){
						write.read(this.writer, node);
					}else {
						res = this.writer(node, write);
						if (res){
	...
	});Module.register('/Users/wl/Sites/TeaJS/lib/syntax/index.js', '../lib/syntax/index.js', function(exports, require, module, __filename, __dirname){
		var token,
			location,
			source,
			regexp,
			match,
			matchNode,
			Node = require("../lib/syntax/node.js"),
			Scope = require("../lib/syntax/scope.js"),
			Parser = require("../lib/syntax/parser.js"),
			SyntaxReg = require("../lib/syntax/regexp.js");
		token = require("../lib/syntax/token.js");
		location = require("../lib/syntax/location.js");
		source = require("../lib/syntax/source.js");
	...
	});Module.register('/Users/wl/Sites/TeaJS/lib/syntax/node.js', '../lib/syntax/node.js', function(exports, require, module, __filename, __dirname){
		var NodeBase = require("../lib/syntax/base.js"),
			Node = (function(){
				function Node(type){
					this.type = type;
					this.length = 0;
		...
		module.exports = Node;
	});Module.register('/Users/wl/Sites/TeaJS/lib/syntax/base.js', '../lib/syntax/base.js', function(exports, require, module, __filename, __dirname){
		var Scope = require("../lib/syntax/scope.js"),
			NodeBase = (function(){
				function NodeBase(){}
				var nodemap = require("../lib/syntax/map.js").node;
	...
	});Module.register('/Users/wl/Sites/TeaJS/lib/syntax/scope.js', '../lib/syntax/scope.js', function(exports, require, module, __filename, __dirname){
		var Scope = (function(){
				function Scope(node){
					this.name = null;
	...
	});Module.register('/Users/wl/Sites/TeaJS/lib/syntax/map.js', '../lib/syntax/map.js', function(exports, require, module, __filename, __dirname){
		var token,
			node,
			token_map = {'LF' : '\n',
				'BlankTokn' : '\r  \t  \f  \ ',
				...
				'BlockStart' : ':  {',
				'Contextual' : 'Binary  Member  Comma  in  as  of  ->  <-  =>  <=  ...  Link',
				'EndTokn' : 'BlockBreakTokn  Close  /*  //',
				'IGLF' : 'Unary  Ternary  Assign  Open  Contextual'},
		...
		token = {"types" : {},
			...
			"define" : function(types, literals){
				...
				if (this.complexs.length){
					this.complexs.sort(function(a, b){
			...
			"define" : function(types, names){
				...
				for (var _i = 0, name; _i < names.length; _i++){
					...
					for (var _j = 0, type; _j < types.length; _j++){
						...
						if (this.map[type].indexOf(name) == -1){
							this.map[type].push(name);
						}
						if (this.all.indexOf(name) == -1){
							this.all.push(name);
						}
					}
				}
	...
	});Module.register('/Users/wl/Sites/TeaJS/lib/syntax/parser.js', '../lib/syntax/parser.js', function(exports, require, module, __filename, __dirname){
		...
		Parser.define('JsonPatt\:4', 'eq?(get set) +1is?(IdentifierTokn Restricted) (eq?(set) SetterDecl | GetterDecl){err:301} | '+'is?(IdentifierTokn Restricted) +1eq(\\\() MethodDecl{err:302} | '+'(:AssignmentExpr is(IdentifierTokn NumTokn StringTokn Restricted) : ExprStam{err:203})');
		...
		Parser.define('LinkExpr', 'AccessorExpr ((?\.\.) (:LinkPatt AccessorExpr ParamsStam?))+');
		...
		Parser.define('StatementStam', 'is?(Controler) ControlClauses() | '+'(eq?({) (JsonExpr|Block()) | Clauses() | Statement() | MethodDecl) (:::SeleteStam SeleteLeftStam)? | '+'LinkExpr | '+'CommaStam (:::CallExpr if(ValueExpr) ParamsStam)? (:::SeleteStam SeleteLeftStam|SeleteRightStam)?');
	...
	});Module.register('/Users/wl/Sites/TeaJS/lib/syntax/regexp.js', '../lib/syntax/regexp.js', function(exports, require, module, __filename, __dirname){
		var Token = require("../lib/syntax/token.js"),
			Node = require("../lib/syntax/node.js"),
			Parser = require("../lib/syntax/parser.js"),
			SyntaxReg = (function(){
				var stx_re_cache = {};
				...
				function matchPattern(ptn, src, __r, __m){
					...
					switch (ptn.type){
						case 'ALL':
							return token;
						case 'RegExp':
							if (ptn.key.test(token.text)){
								return token;
							}
							break;
						case 'Code':
							if (ptn.key == token.text){
								return token;
							}
							break;
						case 'Tokn':
				...
				function matchBad(ptn, src, o_index, __m){
					...
					if (ptn.param.err && !ptn.param.try && !this.try){
						throw tea.error(new Error(), ptn.param.err, err_token, 'Syntax parse error:'+this.source);
					}
					return false;
				}
	...
	});Module.register('/Users/wl/Sites/TeaJS/lib/syntax/token.js', '../lib/syntax/token.js', function(exports, require, module, __filename, __dirname){
		var NodeBase = require("../lib/syntax/base.js"),
			Token = (function(){
				...
				tokenmap = require("../lib/syntax/map.js").token;
	...
	});Module.register('/Users/wl/Sites/TeaJS/lib/syntax/location.js', '../lib/syntax/location.js', function(exports, require, module, __filename, __dirname){
	...
	});Module.register('/Users/wl/Sites/TeaJS/lib/syntax/source.js', '../lib/syntax/source.js', function(exports, require, module, __filename, __dirname){
		var Source = (function(){
				var Splice = Array.prototype.splice,
					...
					Token = require("../lib/syntax/token.js"),
					Location = require("../lib/syntax/location.js"),
					PreProcess = require("../lib/preprocess/index.js");
				...
				Source.prototype.join = function(a, b){
					...
					var texts = [];
					for (var i = a; i <= b; i++){
						if (this[i] && this[i].text != '\4'){
							texts.push(this[i].text);
						}
					}
					return texts.join('');
				};
	...
	});Module.register('/Users/wl/Sites/TeaJS/lib/preprocess/require.js', '../lib/preprocess/require.js', function(exports, require, module, __filename, __dirname){
		var Context = require("../lib/preprocess/context.js");
		function join(requires, main){
			...
			for (var file in requires){
				...
				if (!requires.hasOwnProperty(file)){
					continue;
				}
	...
	});Module.register('/Users/wl/Sites/TeaJS/lib/rewriter/index.js', '../lib/rewriter/index.js', function(exports, require, module, __filename, __dirname){
		var Reader = require("../lib/rewriter/reader.js"),
			SourceMap = require("../lib/rewriter/sourcemap.js"),
			Beautify = require("../lib/rewriter/beautify.js");
	...
	});Module.register('/Users/wl/Sites/TeaJS/lib/rewriter/reader.js', '../lib/rewriter/reader.js', function(exports, require, module, __filename, __dirname){
		var Reader = (function(){
				var Writer = require("../lib/rewriter/writer.js"),
					ES5 = require("../lib/rewriter/ES5.js"),
					pattern_cache = {};
				function Reader(ctx){
				...
				Reader.prototype.patt = function(patt, node, write){
					...
					if (!write){
						write = this.new(patt.name || node.type);
					}
	...
	});Module.register('/Users/wl/Sites/TeaJS/lib/rewriter/writer.js', '../lib/rewriter/writer.js', function(exports, require, module, __filename, __dirname){
		var Beautify = require("../lib/rewriter/beautify.js"),
			Writer = (function(){
	...
	});Module.register('/Users/wl/Sites/TeaJS/lib/rewriter/beautify.js', '../lib/rewriter/beautify.js', function(exports, require, module, __filename, __dirname){
	...
	});Module.register('/Users/wl/Sites/TeaJS/lib/rewriter/ES5.js', '../lib/rewriter/ES5.js', function(exports, require, module, __filename, __dirname){
		...
		ES5.AssignmentExpr = function(node, __write){
			...
			if (node.parent.type == 'JsonExpr'){
				if (node[0].is('IdentifierTokn', 'Keyword')){
					return '"#0"#1 #2';
				}
			}
			return '#0 #1 #2';
		...
		ES5.LinkExpr = function(node, __write){
			for (var i = 1; i < node.length; i++){
				__write.read(this.new('AccessorExpr').read(node[0], '.', node[i]));
				if (i < node.length-1){
					__write.add(';\n');
				}
			}
		...
		ES5.ForStam = function(node, __write){
			...
			var scope = node.scope,
				_ref = ES5.forCondition.call(this, condition),
				$mark = _ref[0],
				$var = _ref[1],
				$i = _ref[2],
				$def = _ref[3],
		...
		ES5.SuperExpr = function(node, __write){
			...
			if (acc.length == 1){
				supe.text += '.'+(node.offsetScope.name || 'constructor');
			}
			if (pam){
		...
		module.exports = ES5;
	});Module.register('/Users/wl/Sites/TeaJS/lib/rewriter/sourcemap.js', '../lib/rewriter/sourcemap.js', function(exports, require, module, __filename, __dirname){
...
var ctx, Tea = require("../lib/tea.js"), ChildProcess = require("child_process");
if (!module.parent){
